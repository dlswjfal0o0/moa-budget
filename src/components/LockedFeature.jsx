...
// 닉네임 옆에 붙는 구독 등급 마크. Pro 구독 중엔 PRO, 아니면(무료체험 포함) 일반으로 표시.
export function SubscriptionBadge({ isSubscribed, style }) {
  if (isSubscribed) {
    return (
      <span style={{
        fontSize: 11, fontWeight: 700, color: '#fff', background: 'rgba(255,255,255,0.25)',
        borderRadius: 9999, padding: '3px 9px', flexShrink: 0, ...style,
      }}>✨ Pro 구독자</span>
    )
  }
  return (
    <span style={{
      fontSize: 11, fontWeight: 700, color: 'rgba(255,255,255,0.85)', background: 'rgba(255,255,255,0.12)',
      borderRadius: 9999, padding: '3px 9px', flexShrink: 0, ...style,
    }}>일반</span>
  )
}
