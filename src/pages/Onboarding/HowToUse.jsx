...
function HomeMockup() {
  ...
  return (
    <div style={{ flex:1, background:'#f5f6f8', overflow:'hidden' }}>
      {/* 헤더 */}
      ...
      <div style={{ padding:'5px 5px 0' }}>
        {/* 예산 관리 */}
        <div style={{ background:'white', borderRadius:16, padding:'7px 8px', marginBottom:4, animation:'moaFadeUp 0.5s 0.2s ease both' }}>
          <div style={{ fontSize:8, fontWeight:600, color:'#111', marginBottom:4 }}>예산 관리</div>
        ...
        {/* 다가오는 결제 */}
        <div style={{ background:'white', borderRadius:16, padding:'7px 8px', marginBottom:4, animation:'moaFadeUp 0.5s 0.35s ease both' }}>
          <div style={{ fontSize:8, fontWeight:600, color:'#111', marginBottom:5 }}>다가오는 결제</div>
          {[
            { title:'월세', days:3, amount:'500,000', color:'#ef4444', bg:'#fee2e2' },
            { title:'티빙', days:8, amount:'13,900', color:'#f59e0b', bg:'#fef3c7' },
          ].map((item,i)=>(
            <div key={i} style={{ display:'flex', alignItems:'center', gap:5, marginBottom:i===0?4:0, background:'#f9f9f9', borderRadius:10, padding:'4px 6px' }}>
              <div style={{ width:20, height:20, borderRadius:8, background:item.bg, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:5.5, color:'#aaa' }}>D-{item.days}</div>
              </div>
        ...
        {/* 카테고리별 소비 */}
        <div style={{ background:'white', borderRadius:16, padding:'7px 8px', animation:'moaFadeUp 0.5s 0.5s ease both' }}>
          <div style={{ fontSize:8, fontWeight:600, color:'#111', marginBottom:5 }}>카테고리별 지출</div>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ position:'relative', width:52, height:52, flexShrink:0, animation:'moaSpinPie 0.8s 0.7s ease both' }}>
              <div style={{ width:52, height:52, borderRadius:'50%', background:`conic-gradient(#FF6B6B 0% 65%, #4ECDC4 65% 80%, #45B7D1 80% 100%)` }}/>
              <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', width:26, height:26, borderRadius:'50%', background:'white' }}/>
            </div>
  ...
  return (
    <div style={{ flex:1, background:'white', overflow:'hidden', animation:'moaFadeUp 0.5s ease both' }}>
      ...
      <div style={{ display:'flex', gap:4, padding:'4px 5px 0' }}>
        <div style={{ flex:1, background:'#FFF5F5', borderRadius:16, padding:'5px 6px' }}>
          <div style={{ fontSize:6, color:'#888' }}>이번 주 지출</div>
          <div style={{ fontSize:8, fontWeight:700, color:'#ef4444' }}>-125,000원</div>
        </div>
        <div style={{ flex:1, background:'#F0FFF4', borderRadius:16, padding:'5px 6px' }}>
      ...
      <div style={{ padding:'4px 5px', borderTop:'1px solid #f5f5f5' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:4 }}>
          {[{n:'월세',d:1,a:'500,000',p:true},{n:'티빙',d:5,a:'13,900',p:false},{n:'보험료',d:10,a:'85,000',p:true},{n:'통신비',d:15,a:'55,000',p:false}].map(item=>(
            <div key={item.n} style={{ background:'#f8f8f8', borderRadius:16, padding:'5px 6px' }}>
...
function LedgerMockup() {
  ...
  let delay = 0.4
  return (
    <div style={{ flex:1, background:'#f5f6f8', overflow:'hidden', position:'relative' }}>
      <div style={{ background:'white', padding:'6px 10px', borderBottom:'1px solid #f0f0f0', animation:'moaFadeUp 0.4s 0.1s ease both' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'3px 0 4px', borderBottom:'1px solid #f5f5f5' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:3 }}>
          <div style={{ display:'flex', gap:5 }}>
            {['전체','소비','수입'].map((t,i)=>(
              <div key={t} style={{ padding:'2px 9px', borderRadius:9999, background:i===0?'#111':'#f0f0f0', fontSize:7, color:i===0?'white':'#666' }}>{t}</div>
            ))}
          </div>
            ...
            {rows.map((item)=> {
              ...
              return (
                <div key={item.t} style={{ background:'white', borderRadius:16, padding:'5px 7px', marginBottom:3, display:'flex', alignItems:'center', gap:5,
                  ...
                  <div style={{ width:22, height:22, borderRadius:12, background:item.col+'22', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
      ...
      <div style={{ position:'absolute', bottom:12, right:12, width:30, height:30, borderRadius:9999, background:PRIMARY, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 2px 8px rgba(0,0,0,0.2)',
...
function AnalysisMockup() {
  ...
  return (
    <div style={{ flex:1, background:'#f5f6f8', overflow:'hidden', padding:'4px 5px' }}>
      {/* 탭 */}
      <div style={{ display:'flex', background:'#f0f0f0', borderRadius:9999, padding:3, marginBottom:4, animation:'moaFadeUp 0.4s ease both' }}>
      ...
      {/* 지난 달 대비 */}
      <div style={{ background:'white', borderRadius:16, padding:'6px 7px', marginBottom:4, animation:'moaFadeUp 0.4s 0.15s ease both' }}>
        <div style={{ fontSize:7, fontWeight:600, color:'#111', marginBottom:3 }}>지난 달 대비</div>
        <div style={{ display:'flex', gap:4 }}>
          <div style={{ flex:1, background:'#FFF5F5', borderRadius:9999, padding:'3px 5px' }}>
            <div style={{ fontSize:5.5, color:'#888' }}>지출</div>
            <div style={{ fontSize:8, fontWeight:700, color:'#ef4444' }}>750,000원</div>
            <div style={{ fontSize:5.5, color:'#ef4444' }}>▲ 5.2%</div>
          </div>
          <div style={{ flex:1, background:'#F0FFF4', borderRadius:9999, padding:'3px 5px' }}>
      ...
      {/* 일별 지출 */}
      <div style={{ background:'white', borderRadius:16, padding:'6px 7px', marginBottom:4, animation:'moaFadeUp 0.4s 0.45s ease both' }}>
      ...
      {/* 카테고리별 지출 */}
      <div style={{ background:'white', borderRadius:16, padding:'6px 7px', marginBottom:4, animation:'moaFadeUp 0.4s 0.7s ease both' }}>
      ...
      {/* AI 분석 */}
      <div style={{ background:'white', borderRadius:16, padding:'6px 7px', animation:'moaScaleIn 0.5s 1.1s ease both' }}>
        ...
        <div style={{ background:'#F0FFF4', borderRadius:9999, padding:'4px 5px', borderLeft:`2px solid #22c55e` }}>
...
function AnalysisUtilityMockup() {
  ...
  return (
    <div style={{ flex:1, background:'#f5f6f8', overflow:'hidden', padding:'4px 5px' }}>
      ...
      {items.map((item,idx)=>(
        <div key={item.name} style={{ background:'white', borderRadius:16, padding:'5px 6px', marginBottom:3,
      ...
      <div style={{ background:'white', borderRadius:16, padding:'5px 6px', animation:'moaScaleIn 0.5s 1.1s ease both' }}>
        ...
        <div style={{ background:'#EEF2FF', borderRadius:9999, padding:'4px 5px', marginTop:4, borderLeft:`2px solid ${PRIMARY}` }}>
  ...
  return (
    <div style={{ flex:1, background:'#f5f6f8', overflow:'hidden' }}>
      ...
      <div style={{ background:PRIMARY, padding:'10px 12px', color:'white', display:'flex', justifyContent:'space-between', alignItems:'center',
        ...
        <div style={{ background:'rgba(255,255,255,0.2)', borderRadius:12, width:28, height:28, display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      <div style={{ padding:'5px 5px 0' }}>
        {/* 카드 실적 */}
        <div style={{ background:'white', borderRadius:16, padding:'7px 8px', marginBottom:4, animation:'moaFadeUp 0.4s 0.35s ease both' }}>
          ...
          {[['신한카드',75,false],['현대카드',92,true]].map(([name,pct,done],i)=>(
            <div key={name} style={{ marginBottom:5 }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:2 }}>
                ...
                {done && <span style={{ fontSize:5.5, background:'#dcfce7', color:'#16a34a', borderRadius:9999, padding:'1px 5px' }}>달성 ✓</span>}
              </div>
              <div style={{ height:4, background:'#f0f0f0', borderRadius:9999, overflow:'hidden' }}>
        ...
        {/* 계좌 */}
        <div style={{ background:'white', borderRadius:16, padding:'7px 8px', marginBottom:4, animation:'moaFadeUp 0.4s 0.6s ease both' }}>
        ...
        {/* 현금 */}
        <div style={{ background:'white', borderRadius:16, padding:'7px 8px', animation:'moaFadeUp 0.4s 0.9s ease both' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
...
function LedgerSettingsMockup() {
  return (
    <div style={{ flex:1, background:'white', overflow:'hidden', padding:'8px 9px' }}>
      ...
      <div style={{ display:'flex', background:'#f0f0f0', borderRadius:9999, padding:3, marginBottom:6 }}>
        <div style={{ flex:1, padding:'4px', borderRadius:9999, background:PRIMARY, textAlign:'center', fontSize:7, color:'white', fontWeight:600 }}>월요일부터</div>
        <div style={{ flex:1, padding:'4px', borderRadius:9999, textAlign:'center', fontSize:7, color:'#888' }}>일요일부터</div>
      </div>
      <p style={{ fontSize:6.5, color:'#888', fontWeight:600, marginBottom:3 }}>정렬 순서</p>
      <div style={{ display:'flex', background:'#f0f0f0', borderRadius:9999, padding:3, marginBottom:6 }}>
        <div style={{ flex:1, padding:'4px', borderRadius:9999, background:PRIMARY, textAlign:'center', fontSize:7, color:'white', fontWeight:600 }}>↓ 최신순</div>
        <div style={{ flex:1, padding:'4px', borderRadius:9999, textAlign:'center', fontSize:7, color:'#888' }}>↑ 오래된순</div>
      </div>
      ...
      <div style={{ height:0.5, background:'#f0f0f0', marginBottom:5 }}/>
      <div style={{ background:'#EEF2FF', borderRadius:16, padding:'6px 8px', border:`1.5px solid ${PRIMARY}44` }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
          <div>
          ...
          <div style={{ width:24, height:13, borderRadius:9999, background:PRIMARY, position:'relative', flexShrink:0 }}>
    ...
    <div style={{ flex:1, background:'white', overflow:'hidden', padding:'8px 8px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:4, marginBottom:7 }}>
        {[
          ...
          <div key={t.n} style={{ background:t.sel?'#EEF2FF':'#f8f8f8', borderRadius:16, padding:'5px 6px', border:t.sel?`1.5px solid ${PRIMARY}`:'1.5px solid transparent' }}>
            <div style={{ display:'flex', alignItems:'center', gap:3, marginBottom:1 }}>
      ...
      {[['📊','엑셀로 내보내기','전체 내역을 .xlsx 파일로'],['📄','PDF로 내보내기','전체 내역을 .pdf 파일로']].map(([icon,label,desc])=>(
        <div key={label} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 0', borderBottom:'1px solid #f8f8f8' }}>
          <div style={{ width:20, height:20, borderRadius:12, background:'#f0f0f0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:9, flexShrink:0 }}>{icon}</div>
          <div>
      ...
      <div style={{ marginTop:8 }}>
        <div style={{ padding:'9px 12px', borderRadius:16, border:'1px solid #FFE4E4', background:'#FFF5F5', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
...
const SLIDES = [
  ...
  { id:'my', tab:'MY', title:'MY', desc:'나의 자산과 앱 설정',
    features:['카드 실적 달성률 · 혜택 확인','계좌 · 현금 잔액 관리','테마 변경 (6가지 스타일)','⚙️ PDF · 엑셀 내보내기 · 로그아웃'],
