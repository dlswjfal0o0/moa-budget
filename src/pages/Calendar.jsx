...
export default function Calendar() {
  ...
  const byDate = transactions.reduce((acc, t) => {
    ...
    if (t.type === 'expense' && !t.cardBilling && !t.isCreditCard && (!showLoan || !t.isLoan)) acc[t.date].expense += t.amount
  ...
  const totalExpense = transactions.filter(t => t.type === 'expense' && !t.cardBilling && !t.isCreditCard && (!showLoan || !t.isLoan)).reduce((s, t) => s + t.amount, 0)
  ...
  const weekExpense = transactions.filter(t => t.type === 'expense' && !t.cardBilling && !t.isCreditCard && (!showLoan || !t.isLoan) && t.date >= weekAgo && t.date <= todayStr).reduce((s, t) => s + t.amount, 0)
            ...
            {selectedTxs.length === 0 ? (
              ...
              selectedTxs.map(t => (
                <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #f8f8f8' }}>
                    ...
                    <p style={{ fontSize: 14, fontWeight: 600, flexShrink: 0, whiteSpace: 'nowrap', color: t.creditCardBilling ? '#ef4444' : (t.cardBilling || t.isCreditCard) ? '#bbb' : (showLoan && t.isLoan) ? (t.type === 'expense' ? '#fca5a5' : '#86efac') : t.type === 'expense' ? '#ef4444' : '#22c55e' }}>
